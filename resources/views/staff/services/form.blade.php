    ...
    <div class="card-body p-4">
        ...
        <form action="{{ isset($service) ? route('staff.services.update', $service) : route('staff.services.store') }}" method="POST" autocomplete="off">
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-md-12">
                    @php
                        $srvStaffUrl = old('project_url', $service->project_url ?? ($service->client->website ?? ''));
                        $clickableStaffSrvUrl = (!empty($srvStaffUrl) && !\Illuminate\Support\Str::startsWith($srvStaffUrl, ['http://', 'https://'])) ? 'https://' . $srvStaffUrl : $srvStaffUrl;
                    @endphp
                    <label class="form-label fw-semibold small text-muted d-flex justify-content-between align-items-center">
                        ...
                        @if(!empty($clickableStaffSrvUrl))
                            <a href="{{ $clickableStaffSrvUrl }}" target="_blank" class="badge bg-primary-subtle text-primary border border-primary-subtle text-decoration-none py-1 px-2">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="project_url" class="form-control bg-light border-start-0" placeholder="e.g. https://myclientproject.com" value="{{ $srvStaffUrl }}">
                        @if(!empty($clickableStaffSrvUrl))
                            <a href="{{ $clickableStaffSrvUrl }}" target="_blank" class="btn btn-outline-primary fw-semibold px-3" title="Visit Live Project">
                                <i class="fa-solid fa-arrow-up-right-from-square me-1"></i> Visit Website
                            </a>
                        @endif
                    </div>
                    <div class="form-text small text-muted">Upload / paste live website or application URL for this project. Accessible by the client in their portal.</div>
                </div>
            </div>
