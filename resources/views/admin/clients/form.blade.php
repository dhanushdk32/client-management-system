    ...
    <div class="card-body p-4">
        ...
        <form id="clientForm" action="{{ isset($client) ? route('admin.clients.update', $client->client_id) : route('admin.clients.store') }}" method="POST" autocomplete="off">
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-md-6">
                    @php
                        $currentUrl = old('website', $client->website ?? ($primaryService->project_url ?? ''));
                        $clickableUrl = (!empty($currentUrl) && !\Illuminate\Support\Str::startsWith($currentUrl, ['http://', 'https://'])) ? 'https://' . $currentUrl : $currentUrl;
                    @endphp
                    <label class="form-label fw-semibold small text-muted d-flex justify-content-between align-items-center">
                        ...
                        @if(!empty($clickableUrl))
                            <a href="{{ $clickableUrl }}" target="_blank" class="badge bg-primary-subtle text-primary border border-primary-subtle text-decoration-none py-1 px-2">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="website" id="projectUrl" class="form-control bg-light border-start-0" placeholder="e.g. https://clientproject.com" value="{{ $currentUrl }}">
                        @if(!empty($clickableUrl))
                            <a href="{{ $clickableUrl }}" target="_blank" class="btn btn-outline-primary fw-semibold px-3" title="Visit Live Project">
                                <i class="fa-solid fa-arrow-up-right-from-square me-1"></i> Visit Website
                            </a>
                        @endif
                    </div>
                    <div class="form-text small text-muted">Upload / paste live website or application URL. The client can click and view this link in their portal.</div>
                </div>
